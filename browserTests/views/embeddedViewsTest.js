...
const loadingIndicator = Selector('[data-sm="LoadingIndicator"]');
  ...
  // Unit view
  test.page`${getBaseUrl()}/fi/embed/unit/148`
  ...
  test.page`${getBaseUrl()}/fi/embed/unit/174?services=961,239&distance=100`
  ...
  test.page`${getBaseUrl()}/fi/embed/search?units=172,187,176`
  ...
  test.page`${getBaseUrl()}/fi/embed/search?service_node=828322864,828322617`
  ...
  // Service view
  test.page`${getBaseUrl()}/fi/embed/service/828322864`
  ...
  // Address view
  test.page`${getBaseUrl()}/fi/embed/address/turku/Aurakatu 3/`
  ('Embedded address view shows nearby service units correctly', async (t) => {
    await t
      .expect(loadingIndicator.exists).notOk('should load data')
      .expect(unitMarkers.count).gt(1, 'Service unit markers should be rendered to map')
      .navigateTo(`${getBaseUrl()}/fi/embed/address/turku/Aurakatu 3/?units=none`)
      .expect(loadingIndicator.exists).notOk('should load data')
  ...
  test.page`${getBaseUrl()}/fi/embed/area?selected=neighborhood&districts=ocd-division/country:fi/kunta:turku/kaupunginosa:001,ocd-division/country:fi/kunta:turku/kaupunginosa:004&services=828322864,828322617`
  ...
  // Division url
  test.page`${getBaseUrl()}/fi/embed/division/kunta:turku/kaupunginosa:002?level=all`
