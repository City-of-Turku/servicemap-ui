...
const ParkAndRideCarsContent = ({ item }) => {
  const intl = useIntl();
  const itemName = {
    fi: item.name_fi,
    en: item.name_en,
      ...
      <div>
        <StyledTextContainer>
          <Typography variant="body2" component="p">
            {intl.formatMessage({ id: 'mobilityPlatform.parkAndRide.content.cars.subtitle' })}
          </Typography>
        </StyledTextContainer>
        {item.address_fi !== '' ? (
...
ParkAndRideCarsContent.propTypes = {
...
ParkAndRideCarsContent.defaultProps = {
  item: {},
};

export default ParkAndRideCarsContent;
